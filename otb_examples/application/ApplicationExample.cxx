...
// \subdoxygen{otb}{Wrapper}{Application} class. We start by including the
// needed header files.
...
public:
  // Class declaration is followed by \code{ITK} public types for the class, the
  // superclass and smart pointers.
  ...
  //  Following macros are necessary to respect ITK object factory mechanisms.
  //  Please report to \ref{sec:FilterConventions} for additional information.

  itkNewMacro(Self);
  itkTypeMacro(ExampleApplication, otb::Application);

private:
  //  \doxygen{otb}{Application} relies on three main private methods:
  //  \code{DoInit()}, \code{DoUpdate()}, and \code{DoExecute()}. Section
  //  \ref{sec:appArchitecture} gives a description a these methods.
  ...
  void DoInit() override {
    ...
    // \item[\code{SetDocLongDescription()}] This methods is used to describe
    // the class.
    // \item[\code{SetDocLimitations()}] Set known limitations (threading,
    // invalid pixel type \dots) or bugs.
    // \item[\code{SetDocAuthors()}] Set the application Authors. Author List.
    // Format : "John Doe, Winnie the Pooh" \dots
    // \item[\code{SetDocSeeAlso()}] If the application is related to one
    // another, it can be mentioned.
    // \end{description}
    ...
    // The header file \code{otbWrapperTags.h} in OTB sources contains some
    // predefined tags defined in \code{Tags} namespace.

    AddDocTag(Tags::Analysis);
    AddDocTag("Test");

    // Application parameters declaration is done using \code{AddParameter()}
    // method.
    // \code{AddParameter()} requires the input parameter type
    // (ParameterType\_InputImage, ParameterType\_Int, ParameterType\_Float),
    // its name and description.
    // \subdoxygen{otb}{Wrapper}{Application} class contains methods to set
    ...
    AddParameter(ParameterType_Directory, "param6",
                 "Example of directory name");
    MandatoryOff("param6");

    AddParameter(ParameterType_Choice, "inchoice",
                 "Example of choice parameter");
    ...
    AddParameter(ParameterType_Int, "ingroup.valint",
                 "Example of integer parameter for group");
    ...
    // \code{SetDocExampleParameterValue()} is used to set parameters. Dataset
    // should be located in  \code{OTB-Data/Examples} directory.
  ...
  // \code{DoUpdateParameters()} is called as soon as a parameter value change.
  // Section \ref{sec:appDoUpdateParameters} gives a complete description of
  // this method.
  void DoUpdateParameters() override {}

  // \code{DoExecute()} contains the application core. Section
  // \ref{sec:appDoExecute} gives a complete description of this method.
